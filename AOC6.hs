...
allParents os = allParents' os os where
    allParents' _ [] _ = []
    allParents' os (o:_) p | outer o == p = (center o) : allParents' os os (center o)
...
solution2 = do
    ...
    let you = Planet "YOU"
        santa = Planet "SAN"
    case orbs of
        Right orbs' -> return $ transferCount (map (calculateDistanceToCom orbs') orbs') you santa
        Left e -> error (show e)
...
prop_transfer =
    ...
    H.withTests 1 $ H.property $
      case parse inputParser "example_transfer" transfer_example of
        Right x -> (transferCount (map (calculateDistanceToCom x) x) you santa) H.=== 4
        Left e -> H.footnote (show e) >> H.failure 

prop_parents :: H.Property
prop_parents =
    let you = Planet "YOU" in
    H.withTests 1 $ H.property $
      case parse inputParser "example_transfer" transfer_example of
        Right x -> (name <$> allParents (fst <$> map (calculateDistanceToCom x) x) you) H.=== ["K", "J", "E", "D", "C", "B", "COM"]
        Left e -> H.footnote (show e) >> H.failure 
