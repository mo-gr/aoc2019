...
buildMachine input = Machine { memory = (convert input), opCode = 0, relBase = 0, input = [], output = [], runState = Running }
...
decodeParameterMode3 x = (toParameterMode . nthDigit 2 $ x, ((toParameterMode . nthDigit 3 $ x), (toParameterMode . nthDigit 4 $ x)))
...
mathOp :: (Value -> Value -> Value) -> ParameterMode -> ParameterMode -> ParameterMode -> MachineState ()
