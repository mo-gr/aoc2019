...
calculateAngle p1 p2 = let half_circle = 180
                           a = (atan2 (fromIntegral $ _x p1 - _x p2) (fromIntegral $ _y p2 - _y p1)) / pi * half_circle
...
magicOrdering origin (a1, p1) (a2, p2) | a1 == a2 = compare (manhattanDistance origin p1) (manhattanDistance origin p2)
...
solution1 = do
    ...
    case pointsOrErrors of
        Right points ->
            return . maximumBy (\a b -> compare (snd a) (snd b)) $ (\p -> (p, countVisibles (sightLines points p))) <$> points
        Left e -> error . show $ e
...
solution2 = do
    ...
    case pointsOrErrors of
        Right points -> do
            let laserPosition = fst $ maximumBy (\a b -> compare (snd a) (snd b)) $ (\p -> (p, countVisibles (sightLines points p))) <$> points
            let twoHundred = nth 200 . sortBy (magicOrdering laserPosition) . offset $ sortBy (magicOrdering laserPosition) (sightLines points laserPosition)
